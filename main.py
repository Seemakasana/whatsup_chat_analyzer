...
if  uploader is not None:
    ...
    user_list.sort()
    # entry2= re.split('([\w\W]+?)and', user_list)
    # if entry2[1:]:
    #     user_list.append(entry2[1])
    #     user_list.append(entry2[2])
    ...
    if st.sidebar.button('show analysis'):
        ...
        if user_df.empty:
            ...
            if selected_user =='overall':
                ...
                with col1:
                    ax.bar(x.index, x.values, color='green', width=0.5)
                    plt.xlabel("users")
                    plt.ylabel("frequency of message")
                    plt.xticks(rotation=45)
                    st.pyplot(fig)
            ...
            with col1:
                fig, ax= plt.subplots()
                ax.bar(common_df[0], common_df[1], width=0.5)
